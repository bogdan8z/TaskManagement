...
const mockState = {
  ...
  tasks: [
    { id: 1, title: 'Mock task 1', completed: false, createdAt: new Date(), userId: 1, priority: 'medium', dueDate: addDaysToDate(new Date(), 1) },
    { id: 2, title: 'Mock task 2', completed: true, createdAt: new Date(), userId: 1, priority: 'high', dueDate: addDaysToDate(new Date(), 2) },
    { id: 3, title: 'Mock task 3', completed: true, createdAt: new Date(), userId: 1, priority: 'low', dueDate: addDaysToDate(new Date(), 3) },
  ],
  nextTaskId: 4,  
}
...
async function request(path, options = {}) {
  if (USE_MOCK) {
    ...
    if (path === '/api/auth/login' && (options.method || 'POST').toUpperCase() === 'POST') {
      ...
      if (!user) {
        user = { id: mockState.users.length + 1, email }
    ...
    if (path === '/api/auth/register' && (options.method || 'POST').toUpperCase() === 'POST') {
      const body = options.body ? JSON.parse(options.body) : {}
      const email = body.email || `user${mockState.users.length + 1}@local`
      const user = { id: mockState.users.length + 1, email }
      mockState.users.push(user)
      return { token: 'mock-token', user }
    }
